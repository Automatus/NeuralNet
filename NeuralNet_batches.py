...
def autolearn(file, datafolder):
    ...
    repeat = int(input("How manu times should the proces be repeated? ('epochs') (for example 1)"))
    batchsize = int(input("Batchsize (for example 16 or 32):"))
...
#    errorvalues = []
    ...
    wi = npzfile["arr_1"]  # input to first layer weigths
    w = npzfile["arr_2"]  # weights between layers
    wo = npzfile["arr_3"]  # last layer to output weigths

    # these are arrays that store the adjustments for the weigths for every batch
    # in index 0 is the summ of adjustments and in index 1 is the amount of adjustments
    # after each input file or batch (now input file) the average is calculated and added to the original weigths
    wt = np.zeros((number_of_hidden_layers - 1, number_of_neurons_in_layer, number_of_neurons_in_layer, 2))
    wit = np.zeros((number_of_inputs, number_of_neurons_in_layer, 2))
    wot = np.zeros((number_of_neurons_in_layer, number_of_outputs, 2))
...
#                plotit(errorvalues)
                if 1 not in neurons[-1, :] and item:
                            ...
                            if j == (-number_of_hidden_layers - 1) and np.any(e > 0 for e in inputs):
                                ...
                                for bla in inputs[0, :]:  # iterating inputs
                                    if bla > 0:  # if input is active
                                        r = 0
                                        for thing in wit[t, :, 0]:
                                            wit[t, r, 0] = (wit[t, r, 0] + step / number_of_inputs * bla * randomvalues[0, r])
                                            wit[t, r, 1] = wit[t, r, 1] + 1
                                ...
                                for neuron in neurons[j, :]:  # iterating neurons
                                    if neuron:  # if neuron is active
                                        r = 0
                                        for thingy in wt[j + 1, :, t, 0]:
                                            wt[j + 1, r, t, 0] = wt[j + 1, r, t, 0] + (step / number_of_neurons_in_layer * randomvalues[0, r])
                                            wt[j + 1, r, t, 1] = wt[j + 1, r, t, 1] + 1
                    ...
                    for neuron in neurons[j, :]:
                        if neuron:  # if neuron is firing
                            wot[k, i, 0] = wot[k, i, 0] + f_error * (step / number_of_neurons_in_layer)
                            wot[k, i, 1] = wot[k, i, 1] + 1
                            ...
                            if neuron:  # if neuron fired
                                ...
                                for thing in spll[0, :]:
                                    ...
                                    if spll[0, z]:  # if neuron spiked
                                        wt[j + 1, z, k, 0] = wt[j + 1, z, k, 0] + (f_error * step / number_of_neurons_in_layer)
                                        wt[j + 1, z, k, 1] = wt[j + 1, z, k, 1] + 1
                                        # change weight strenght in proportion to
                                        # error and number of neurons
                                    z += 1
                            k += 1
                        ...
                        if innn > 0:  # if input is 1
                            ...
                            for it in spll[0, :]:
                                ...
                                if spll[0, q]:  # if neuron spiked
                                    wit[x, q, 0] = wit[x, q, 0] + f_error * innn * step
                                    wit[x, q, 1] = wit[x, q, 1] + 1
                                    # change weight strenght in proportion to
            ...
            if ibatch == batchsize:
                ...
                ibatch = 0
                #  now the average change in weights is taken per input file (not per batch)
                #  'where' src: https://stackoverflow.com/questions/43867102/division-with-numpy-matrices-that-might-result-in-nan
                w = w + np.divide(wt[:, :, :, 0], wt[:, :, :, 1], where=((wt[:, :, :, 1] != 0) | (wt[:, :, :, 0] != 0)))
                wi = wi + np.divide(wit[:, :, 0], wit[:, :, 1], where=((wit[:, :, 1] != 0) | (wit[:, :, 0] != 0)))
                wo = wo + np.divide(wot[:, :, 0], wot[:, :, 1], where=((wot[:, :, 1] != 0) | (wot[:, :, 0] != 0)))
                #  arrays for change in weights are resetted
                wt[:, :, :, :] = 0
                wit[:, :, :] = 0
                wot[:, :, :] = 0
        repeating += 1
